...
public class SemanticPass extends VisitorAdaptor {
	...
	public void visit(Program program) {
		...
		Tab.closeScope();
		if (mainFound == false) {
			report_error("Metoda void main() mora biti deklarisana", null);
		}
	}
	...
	public void visit(MethodTypeName methodTypeName) {
		...
		if (methodTypeName.getI2().equals("main")) {
			...
			/*
			 * if (!noArguments) {
			 * report_error("Funkcija sa nazivom main mora biti bez argumenata",
			 * methodTypeName); }
			 */
			mainFound = true;
		}
	...
	public void visit(DesignArr designArr) {
		...
		designArr.obj = new Obj(Obj.Elem, designArr.getDesignator().obj.getName(),
				designArr.getDesignator().obj.getType().getElemType());

		Struct varArrStruct = designArr.getDesignator().obj.getType();
		if (varArrStruct.getKind() != Struct.Array) {
			designArr.obj = Tab.noObj;
			report_error(designArr.getDesignator().obj.getName() + " nije niz", designArr);
		}
	...
	public void visit(TermFactorial term) {
		Struct tStruct = term.getTerm().struct;
		if (tStruct != Tab.intType) {
			report_error("Term i mulop moraju oba biti tipa int", term);
			term.struct = Tab.noType;
		} else {
			term.struct = Tab.intType;
				...
				if (args.getLevel() != fun.getLevel()) {
					...
					for (Iterator iterator = formalArgs.iterator(); iterator.hasNext();) {
						...
						if (!objFormal.getType().assignableTo(objActual.getType())) {
							report_error("Pogresan tip argumenta", funcCall);
						}
					}
				}
			}
